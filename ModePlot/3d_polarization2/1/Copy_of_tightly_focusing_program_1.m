...
%% Linear fitting
figure(2)
col1 = [1471.*ratio 1524.*ratio];
row1 = [1580.*ratio 1428.*ratio];
col2 = [1443.*ratio 1566.*ratio];
row2 = [1473.*ratio 1530.*ratio];
...
boundary_x1(1)=1485.*ratio; boundary_y1(1)=boundary_x1(1)*slope1+intercept1;
boundary_x2(1)=1473.*ratio; boundary_y2(1)=boundary_x2(1)*slope2+intercept2;

boundary_x1(2)=1494.5.*ratio; boundary_y1(2)=boundary_x1(2)*slope1+intercept1;
boundary_x2(2)=1495.*ratio; boundary_y2(2)=boundary_x2(2)*slope2+intercept2;

boundary_x1(3)=1503; boundary_y1(3)=boundary_x1(3)*slope1+intercept1;
boundary_x2(3)=1514.*ratio; boundary_y2(3)=boundary_x2(3)*slope2+intercept2;

boundary_x1(4)=1512.5; boundary_y1(4)=boundary_x1(4)*slope1+intercept1;
boundary_x2(4)=1538.*ratio; boundary_y2(4)=boundary_x2(4)*slope2+intercept2;
...
Exf=Ex_inff(round(L/2+1-xmax2):round(L/2+1+xmax2),round(L/2+1-xmax2):round(L/2+1+xmax2)); %cropped
%Exf=Ex_inff;
...
Eyf=Ey_inff(round(L/2+1-xmax2):round(L/2+1+xmax2),round(L/2+1-xmax2):round(L/2+1+xmax2));
%Eyf=Ey_inff;
...
Ezf=Ez0f(round(L/2+1-xmax2):round(L/2+1+xmax2),round(L/2+1-xmax2):round(L/2+1+xmax2));
...
figure(7)
subplot(2,4,1);imagesc(Et_n);axis image; colorbar('Ticks',[0 1],'TickLabels',{0,1}); title('Total Intensity');axis off; clim([0 1]);
subplot(2,4,2);imagesc(Ex_n);axis image; colorbar('Ticks',[0 round(max(max(Ex_n)),2)],'TickLabels',{0,round(max(max(Ex_n)),2)}); title('x');axis off; clim([0 round(max(max(Ex_n)),2)]);
subplot(2,4,3);imagesc(Ey_n);axis image; colorbar('Ticks',[0 round(max(max(Ey_n)),3)],'TickLabels',{0,round(max(max(Ey_n)),3)}); title('y');axis off; clim([0 round(max(max(Ey_n)),3)]);
subplot(2,4,4);imagesc(Ez_n);axis image; colorbar('Ticks',[0 round(max(max(Ez_n)),2)],'TickLabels',{0,round(max(max(Ez_n)),2)}); title('z');axis off; clim([0 round(max(max(Ez_n)),2)]);
...
fontsize(7,22,"points");colormap('jet');
