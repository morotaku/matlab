...
M=5;n1=3; n2=3;
N=600*sqrt(M); % number of points for the aperture radius
range=110000; %display range
fig_size=300; %Inputビームの画像サイズ変更
L=5400; 
...
imp1=importdata('0.csv'); imp1=imresize(imp1,[fig_size fig_size]);
imp_x=abs(sqrt(imp1)); imp_x=imp_x./max(max(imp_x));
imp_x=g_filter(imp_x,10,5);

imp2=importdata('90.csv'); imp2=imresize(imp2,[fig_size fig_size]);
imp_y=abs(sqrt(imp2)); imp_y=imp_y./max(max(imp_y));
...
boundary_x1(1)=2693; boundary_y1(1)=boundary_x1(1)*slope1+intercept1;
boundary_x2(1)=2682; boundary_y2(1)=boundary_x2(1)*slope2+intercept2;

boundary_x1(2)=2704; boundary_y1(2)=boundary_x1(2)*slope1+intercept1;
boundary_x2(2)=2704; boundary_y2(2)=boundary_x2(2)*slope2+intercept2;

boundary_x1(3)=2714; boundary_y1(3)=boundary_x1(3)*slope1+intercept1;
boundary_x2(3)=2724; boundary_y2(3)=boundary_x2(3)*slope2+intercept2;
...
figure(7)
subplot(2,4,1);imagesc(Et_n);axis image;  title('Total Intensity');axis off; clim([0 1]); %colorbar('Ticks',[0 1],'TickLabels',{0,1});
subplot(2,4,2);imagesc(Ex_n);axis image;  title('x');axis off; clim([0 round(max(max(Ex_n)),2)]); %colorbar('Ticks',[0 round(max(max(Ex_n)),2)],'TickLabels',{0,round(max(max(Ex_n)),2)});
subplot(2,4,3);imagesc(Ey_n);axis image;  title('y');axis off; clim([0 round(max(max(Ey_n)),3)]); %colorbar('Ticks',[0 round(max(max(Ey_n)),3)],'TickLabels',{0,round(max(max(Ey_n)),3)});
subplot(2,4,4);imagesc(Ez_n);axis image;  title('z');axis off; clim([0 round(max(max(Ez_n)),2)]); %colorbar('Ticks',[0 round(max(max(Ez_n)),2)],'TickLabels',{0,round(max(max(Ez_n)),2)});
