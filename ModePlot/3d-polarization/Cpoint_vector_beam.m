...
%%% LG mode
LG_n1=LGmode(p1,l1,r2,phi2,z1,w1,lam);

%%% HGB
LG_n2=LGmode(p1,l2,r2,phi2,z1,w2,lam).*exp(-1j.*l2.*phi2);
...
for t = 0:120
    ...
    E1_1=sin(thita/2)*exp(1j*phi/2).*exp(1j.*(l1.*phi2(g1))).*exp(1j*t*pi/60).*abs(LG_1);  %ℓ=1
    %%%South pole
    E2_1=cos(thita/2)*exp(-1j*phi/2).*exp(1j.*(0.*phi2(g1))).*exp(1j*t*pi/60).*abs(HGB_1); %ℓ=-1
    %%%|LHC> ℓ=1
    ex1_1=real(E1_1);
    ey1_1=real(-1j.*E1_1);
    %%%|RHC> ℓ=-1
...
for t = 0:120
    %%%North pole of Poincare sphere
    E1_2=sin(thita/2)*exp(1j*phi/2).*exp(1j.*(l1.*phi2(g2))).*exp(1j*t*pi/60).*abs(LG_2);
    %%%South pole
    E2_2=cos(thita/2)*exp(-1j*phi/2).*exp(1j.*(l2.*phi2(g2))).*exp(1j*t*pi/60).*abs(HGB_2); %ℓ=-1
    %%%|LHC> ℓ=1
    ex1_2=real(E1_2);
    ey1_2=real(-1j.*E1_2);
    %%%|RHC> ℓ=-1
