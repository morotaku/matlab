...
Ex=1;%LGmode(0,l1,r,phi,0,1,lam);%exp(1j.*l1.*phi);
% %Ey=1j.*(LGmode(0,l1,r,phi,0,1,lam)-LGmode(0,l2,r,phi,0,1,lam));%.*LGmode(0,l2,r,phi,0,1,lam);
Ey=1;%0.*LGmode(0,l1,r,phi,0,1,lam);%.*exp(1j.*l1.*phi);
% l1=0;
% l2=0;
b=1.2;
eX_x1=Ex.*i^l1.*(I0(l1,l1,alpha,k,z,phi,r,b)+I1(l1,l1+2,alpha,k,z,phi,r,b).*exp(2.*1j.*phi)+I1(l1,l1-2,alpha,k,z,phi,r,b).*exp(-2.*1j.*phi));
eX_x2=Ex.*i^l2.*(I0(l2,l2,alpha,k,z,phi,r,b)+I1(l2,l2+2,alpha,k,z,phi,r,b).*exp(2.*1j.*phi)+I1(l2,l2-2,alpha,k,z,phi,r,b).*exp(-2.*1j.*phi));
eX_y1=Ey.*i^(l1+1).*(-I1(l1,l1+2,alpha,k,z,phi,r,b).*exp(2.*1j.*phi)+I1(l1,l1-2,alpha,k,z,phi,r,b).*exp(-2.*1j.*phi));
eX_y2=Ey.*i^(l2+1).*(-I1(l2,l2+2,alpha,k,z,phi,r,b).*exp(2.*1j.*phi)+I1(l2,l2-2,alpha,k,z,phi,r,b).*exp(-2.*1j.*phi));
eX=(eX_x1+eX_x2)+(1j.*eX_y1-1j.*eX_y2);

eY_x1=Ex.*1j^(l1+1).*(-I1(l1,l1+2,alpha,k,z,phi,r,b).*exp(2.*1j.*phi)+I1(l1,l1-2,alpha,k,z,phi,r,b).*exp(-2.*1j.*phi));
eY_x2=Ex.*1j^(l2+1).*(-I1(l2,l2+2,alpha,k,z,phi,r,b).*exp(2.*1j.*phi)+I1(l2,l2-2,alpha,k,z,phi,r,b).*exp(-2.*1j.*phi));
eY_y1=Ey.*1j^l1.*(I0(l1,l1,alpha,k,z,phi,r,b)-I1(l1,l1+2,alpha,k,z,phi,r,b).*exp(2.*1j.*phi)-I1(l1,l1-2,alpha,k,z,phi,r,b).*exp(-2.*1j.*phi));
eY_y2=Ey.*1j^l2.*(I0(l2,l2,alpha,k,z,phi,r,b)-I1(l2,l2+2,alpha,k,z,phi,r,b).*exp(2.*1j.*phi)-I1(l2,l2-2,alpha,k,z,phi,r,b).*exp(-2.*1j.*phi));
eY=(eY_x1+eY_x2)+(1j.*eY_y1-1j.*eY_y2);

eZ_x1=Ex.*1j.^(l1-1).*(I2(l1,l1+1,alpha,k,z,phi,r,b).*exp(1j.*phi)-I2(l1,l1-1,alpha,k,z,phi,r,b).*exp(-1j.*phi));
eZ_x2=Ex.*1j.^(l2-1).*(I2(l2,l2+1,alpha,k,z,phi,r,b).*exp(1j.*phi)-I2(l2,l2-1,alpha,k,z,phi,r,b).*exp(-1j.*phi));
eZ_y1=Ey.*1j.^l1.*(-I2(l1,l1+1,alpha,k,z,phi,r,b).*exp(1j.*phi)-I2(l1,l1-1,alpha,k,z,phi,r,b).*exp(-1j.*phi));
eZ_y2=Ey.*1j.^l2.*(-I2(l2,l2+1,alpha,k,z,phi,r,b).*exp(1j.*phi)-I2(l2,l2-1,alpha,k,z,phi,r,b).*exp(-1j.*phi));
eZ=(eZ_x1+eZ_x2)+(1j.*eZ_y1-1j.*eZ_y2);
...
function y=I0(l1,l2,alpha,k,z,phi,r,f)
    ...
    for t=0:100
        ...
        thita=w*t;
        r_int=f.*tan(thita);
        A=LGmode(0,l1,r_int,phi,0,1,1.064);
        I=I+sqrt(cos(thita)).*sin(thita).*(1+cos(thita)).*besselj(l2,k.*r.*sin(thita)).*exp(1j.*k.*z.*cos(thita)).*A;
    end
    y=-1j*k.*1700.*I;
end

function y=I1(l1,l2,alpha,k,x,y,z,f)
    I=0;
    for t=0:100
        w=alpha/100;
        thita=w*t;
        r_int=f.*tan(thita);
        A=HGmode(n,m,x,y,w,z,R,k);
        I=I+sqrt(cos(thita)).*sin(thita).*(1-cos(thita)).*besselj(l2,k.*r.*sin(thita)).*exp(1j.*k.*z.*cos(thita)).*A;
    end
    y=-1j*k.*1700.*I/2;
end
