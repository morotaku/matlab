...
for t=0:60
    ...
    thita=w*t;
    r_a=f.*tan(alpha);
    Ex=LGmode(0,l1,r_a,phi,0,w1,lam);
    A=1j*pi/lam.*sqrt(cos(thita)).*sin(thita).*exp(-1j.*k.*z.*cos(thita));
    J0=besselj(l1,k.*r.*sin(thita));
    J1=besselj(l1+1,k.*r.*sin(thita));  
    J2=besselj(l1+2,k.*r.*sin(thita));
    aX=A.*(((1+cos(thita)).*J0+(1-cos(thita)).*J2.*cos(2.*phi)).*Ex+(1-cos(thita)).*J2.*sin(2.*phi).*Ey);
    aY=A.*(((1+cos(thita)).*J0-(1-cos(thita)).*J2.*cos(2.*phi)).*Ey+(1-cos(thita)).*J2.*sin(2.*phi).*Ex);
    aZ=2.*A.*(1j.*sin(thita).*J1.*cos(phi).*Ex+1j.*sin(thita).*J1.*sin(phi).*Ey);
    %aZ=sqrt(cos(thita)).*sin(thita).^2.*J1;
    eX=eX+(aX).*w;
    eY=eY+(aY).*w;
    eZ=eZ+(aZ).*w;
end
...
function y=Bessel(m,x)
    j=0;
    for k = 0: 1000
        j=j+(-1^k).*(x./2).^(m+2*k)./(factorial(k).*gamma(m+k+1));
    end
    y=j;
end