%% Example of the Ince_Gaussian function
%{
This script plots the first 25 even/odd Ince Gaussian Beams 
using the Ince_Gaussian function:
[IGB,X,Y]=Ince_Gaussian(L,N,parity,p,m,e,w0,k,z)
%}
close all;
%% Ellipticity 
ec=100;  % Change the ellipticity to see how the Ince Gaussian Modes behave, for example:
...
p=3;m=3;
% [IGB,X,Y]=Ince_Gaussian(15e-3,201,0,p,m,ec,3e-3,(2*pi/632.8e-9),0); 
% figure(1)
% imagesc(abs(IGB)); shading interp; lighting phong; view(2); axis equal; axis tight;  axis off;
% figure(2)
% imagesc(angle(IGB))
% 
% IGB2=rot45(abs(IGB));
...
for p=1:8
    for m=(2:2:(p+mod(p,2)*2))-mod(p,2)
        [IGB,X,Y]=Ince_Gaussian(15e-3,201,1,p,m,ec,3e-3,(2*pi/632.8e-9),0); 
        hax=axes();
        surf(X,Y,abs(IGB)); shading interp; lighting phong; view(2); axis equal; axis tight;  axis off;
        rowIdx=fix((count-1)/5); colIdx=mod(count-1,5); newPos=[.02+.16*colIdx,0.7-.2*rowIdx,.25,.25]; set(gca,'outer',newPos),
        count=count+1;
