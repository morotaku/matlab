...
%%% LG mode
LG_i=LGmode(p,l,r,phi1,z,w,lam);
E1_i=sin(thita/2)*exp(1j*phi/2).*exp(1j.*(l.*phi1)).*LG_i; % ℓ=ℓ
E2_i=cos(thita/2)*exp(-1j*phi/2).*exp(1j.*(-l.*phi1)).*LG_i; % ℓ=-ℓ
...
ex2_i=real(E2_i);
ey2_i=real(1j.*E2_i);
[Ex_i,Ey_i]=polarizer(angle,ex1_i+ex2_i,ey1_i+ey2_i);
...
%%% LG mode
LG_q=LGmode(p,l,r2,phi2,z,w,lam);
...
for t = 0:200
    ...
    E1_q=sin(thita/2)*exp(1j*phi/2).*exp(1j.*(l.*phi2+t*o_q)).*LG_q;  %ℓ=1
    %%%South pole
    E2_q=cos(thita/2)*exp(-1j*phi/2).*exp(1j.*(-l.*phi2+t*o_q)).*LG_q; %ℓ=-1
    %%%|LHC> ℓ=1
    ex1_q=real(E1_q);
    ey1_q=real(-1j.*E1_q);
    %%%|RHC> ℓ=-1
