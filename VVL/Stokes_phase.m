...
e_v=hgmode(4,0,x,y,w,z,R,k).*exp(1j.*pi/2);
...
s0_n=s0./max(max(s0));
s1_n=s1./max(max(s0));
s2_n=s2./(max(max(s0)));s2_n(s2_n<0.000001)=0;
s3_n=s3./max(max(s0));
...
Stp12 = atan(s2_n./s1_n); % atan関数を使用して角度を計算（各要素ごとに計算）
Stp12 = Stp12 + pi*(s2_n < 0 & s1_n >= 0); % xが負かつyが非負の場合に角度を補正
Stp12 = Stp12 + 2*pi*(s1_n < 0); % yが負の場合に角度を補正
