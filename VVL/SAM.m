...
%cx=ones(L,L); cy=1i*ones(L,L); % lin diag
cx=1i*ones(L,L); cy=1*ones(L,L); % lin diag
%cx=1i.*ones(L,L); cy=ones(L,L); %%right circ
%cx=ones(L,L); cy=1i.*ones(L,L); %%left circ
...
figure(5)
colormap('parula');
Exf_n=Exf./max(max(abs(Exf)));
Eyf_n=Eyf./max(max(abs(Eyf)));
Ezf_n=Ezf./max(max(abs(Ezf)));
sam_x=imag(conj(Eyf).*Ezf-conj(Ezf).*Eyf);sam_xn=sam_x./max(max(abs(sam_x)));
sam_y=imag(conj(Ezf).*Exf-conj(Exf).*Ezf);sam_yn=sam_y./max(max(abs(sam_y)));
sam_z=imag(conj(Exf).*Eyf-conj(Eyf).*Exf);sam_zn=sam_z./max(max(abs(sam_z)));

max(max(sam_z))
min(min(sam_z))
subplot(1,3,1);imagesc(sam_xn);axis image; colorbar;axis off;clim([-1 1]);
subplot(1,3,2);imagesc(sam_yn);axis image; colorbar;axis off;clim([-1 1]);
subplot(1,3,3);imagesc(sam_zn);axis image; colorbar;axis off;clim([-1 1]);
colorbar();
fontsize(5,30,"points");
 %
